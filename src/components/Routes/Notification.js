...
export function notificationFunc(book, user, msg, type, commentid, senderid, enabled){
const  curentuser = firebase.auth().currentUser
 ...
 if(enabled){
  if(type==='likepost'){
    if(book.userid!==user){
    db.collection('notifications').doc(book.userid).update({
  ...
  if(type==='follow'){
    db.collection('notifications').doc(user).update({
