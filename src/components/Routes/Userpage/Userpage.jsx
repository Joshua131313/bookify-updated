...
function Userpage(props){
  const {allbooksdoc} = useContext(ContextApp)
    ...
    useEffect(()=>{
      db.collection('allbooks').doc(allbooksdoc).onSnapshot(snap=>{
      ...
      <Userpagediv userisfollowing={userisfollowing} user={user} userinfo={usersdata} userbookslength={usersbooks.length} currentuserinfo={currentuserinfo}/>
      <Userroutes userisfollowing={userisfollowing}following={following} followers={followers} setFollowers={setFollowers} setFollowing={setFollowing} usersinfo={usersdata} usersbooks={usersbooks} user={user}/>
    </div></>
    }/>
}
