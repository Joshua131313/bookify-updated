...
function User(props) {
  ...
  function Unfollow(){
    notificationFunc('', user, 'unfollowed you.','follow', '', currentuser.uid, true)
  ...
  function follow(){
    notificationFunc('', user, 'followed you.','follow', '', currentuser.uid, true)
    db.collection('users').doc(currentuser.uid).update({
 ...
 useEffect(()=>{
  ...
  return (
    <Link smooth className='profilepic' to={`/${user}#followers`}>
        <div style={!follower&&!following?{width: '100%'}:{width: ''}} className={!follower&&!following?'flexrow ac sb':'flexrow ac'}>
       ...
       <div className="button">
         ...
         {includes && includes.includes(user)?<button className='bannerBtn' style={{marginTop: '0'}} onClick={()=>Unfollow()}>
              Unfollow
          </button>:<button className='bannerBtn' onClick={()=>follow()} style={{marginTop: '0'}} >Follow</button>}
         </Link>
        </div>}
    </Link>
  )
