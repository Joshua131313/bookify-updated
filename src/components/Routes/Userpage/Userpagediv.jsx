...
function Userpagediv(props){
  const {userinfo, userbookslength, user, currentuserinfo, userisfollowing} = props
  const currentuser = firebase.auth().currentUser
  function sendFollowRequest(){
      notificationFunc('', user.uid, 'followed you.','follow', '', currentuser.uid, true)
  ...
  function unFollow(){
    notificationFunc('', user.uid, 'unfollowed you.','follow', '', currentuser.uid, true)
    db.collection('users').doc(currentuser.uid).update({
  ...
  return <div className="userdiv">
      <div className="profileimg" style={{marginTop: '25px'}}>
       ...
       <div className="pdng"> <img src={userinfo.cover} alt=""/></div>
        <h3>{userinfo.name}</h3>
      </div>
      <div className="generalinfo flexrow">
        <span><span>Followers</span><strong>{userisfollowing && userisfollowing.length}</strong></span>
        <hr className="vertical"/>
        <span>
          <span>Post</span>
          <strong className="length">{userbookslength}</strong>
        </span>
        <hr className="vertical"/>
        <span>
          <span>Comments</span>
    ...
    currentuserinfo.following && currentuserinfo.following.includes(user.uid)?
     <button className='bannerBtn' onClick={()=>unFollow()}>
       <i className="fal fa-user-minus"></i> 
       Unfollow
     </button>
     :
    <button className="bannerBtn" onClick={()=>{sendFollowRequest()}}>
