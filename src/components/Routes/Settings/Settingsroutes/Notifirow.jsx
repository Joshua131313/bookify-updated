...
function Notifirow(props){
  ...
  const staterow = state&&state.map(el=>{
  ...
  return <div className='overflow'>
    ...
    <div style={{margin: '20px 0'}}>
      <Switchdiv state={clearnotifisstate}  fnct={clearNotifis} title={'Clear Notifications:'}/>
    </div>
    <div style={{margin: '20px 0'}}>
      <Switchdiv state={recnotifis} setState={setRecnotifis} fnct={endisNotifis} title={'Receive Notifications:'}/>
    </div>
    {staterow && staterow.length===0?<Defaultmsg link='/home' text='No Notifications' icon='fal fa-bell-slash'/>: <div className="msgs">{staterow}</div>}
    </div> 
