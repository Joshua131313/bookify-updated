...
export default function optionEdit(text, setDisabled, setDropmenu, comments, disabled, comment, setNotifibool, setNotification, book, user) {
  ...
  comments && comments.map(commentm=>{
    if(commentm.commentid===comment.commentid){
      ...
      db.collection('users').doc(user.uid).update({
        hiddenmsgs: firebase.firestore.FieldValue.arrayUnion(comments[commentIndex])
      })
    }
  })
}
