...
function Comment (props) {
  ...
  const { notification, setNotification, notifibool, setNotifibool} =useContext(ContextApp)
  const msgoptions = [
  {
    icon: 'fal fa-edit',
    text: 'Edit'
...
},
{
      ...
      comments && comments.map(commentm=>{
          if(!commentm.likedby.includes(user.uid)){
            if(commentm.commentid===comment.commentid){
              ...
              notificationFunc(book, user.uid, 'liked your comment.', 'likecomment', comment.commentid, commentm.senderid, enabled)
            }
          }else {
            if(commentm.commentid === comment.commentid){
              let commentIndex = comments.indexOf(commentm)
              ...
              notificationFunc(book, user.uid, 'unliked your comment.', 'likecomment', comment.commentid, commentm.senderid, enabled)
  ...
  return <>
    {comment.hide?'':
      <Individcomment comments={comments} comment={comment} editmsgstring={editmsgstring} disabled={disabled} setDisabled={setDisabled} setNotifibool={setNotifibool} likes={likes} setEditmsgstring={setEditmsgstring} saveMsg={saveMsg} liked={liked}  dropmenu={dropmenu} setDropmenu={setDropmenu} msgoptions={msgoptions}  userimg={userimg} book={book}/>
      }
  </>  
  }
