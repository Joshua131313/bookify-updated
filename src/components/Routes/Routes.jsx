...
function Routes(props){
  ...
  const {allusers, setAllusers, add, setAdd, keyword, setKeyword, setLoader, setHide, setNotifibool, setNotification} = useContext(ContextApp)
  const [books, setBooks] = useState([])
  const [usersbooks, setUsersbooks] = useState([])
  const [favorites, setFavorites] = useState([])
  const [userbks, setUserbks] = useState([])

  const allbooksroute = books && books.map(book=>{
    return <Route path={`/books/${book.bookid}`}>
    <Bookpage book={book} books={books} favorites={favorites}/>
 </Route>

  })
  const allusersroute = allusers && allusers.map(userm=>{
    return <Route path={`/${userm.uid}`}>
  ...
  useEffect(()=>{
    db.collection('allbooks').doc('OwDcz5n4YBd9MvoXtrik').onSnapshot(snap=>{
      const booksdata = snap.data()
      setBooks(booksdata.books)   
    })
    // db.collection('books').doc(user.uid).onSnapshot(snap=>{
    //   const booksdata = snap.data()
    ...
    <AnimatePresence>
        <Switch location={location} >
          ...
          <Route path='/library'>
            ...
            carousel={true}
             banner='http://unblast.com/wp-content/uploads/2018/09/3.jpg' 
             favorites={favorites} 
          ...
          <Route path='/premiumpost'>
            <Defaultmsg text='Premium Post, get pro to gain access to this post.' link='/pro' icon='fal fa-exclamation-circle'/>
          </Route>
          <Route path='/pro'>
            <Pro />
          </Route>
