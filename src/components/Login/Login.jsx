...
function Login(props){
  const {loginwithFacebook,loginwithGoogle,setlName,lname,name, setName, email, setEmail, password, setPassword, handleLogin, handleSignup, hasAccount, setHasAccount, emailError, passwordError } = props
  ...
  return (
    ...
    <div className="title">
    <h1 className='flexrow' style={{alignItems: 'center'}}><img className='mainlogo' src="https://i.imgur.com/y5bNwaF.png" alt=""/>
      {hasAccount?'Log In':'Register'}</h1>
    </div>
  ...
  <div className='loginBanner flex box-shadow-br'>
    ...
    <div className='flex abs loginbtns'>
      <button className='login-btn gog-btn br sa' onClick={loginwithGoogle()}> <i className="fab fa-google"></i> Login with Google</button>
      <button className='login-btn fb-btn br sa' onClick={loginwithFacebook()}><i className="fab fa-facebook"></i> Login with Facebook</button>
     </div>
     <img className='br' src="https://i.imgur.com/alwIus7.jpg" alt=""/>
    </div>
  </div>
      </div>
     </Route>
