...
function Label(props){
  ...
  return  (
    <label className={active?'active labelicon':'labelicon'} onClick={()=>{fnct && fnct();setKeyword && setKeyword(text.toLowerCase()); setFavoritesbool(state2&&state2); setMore && setMore(prev=>!prev)}}>
      <i className={icon}></i>
      <p>{text}</p>
    </label>
  )
