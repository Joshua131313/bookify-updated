...
function Body(props){
  ...
  const {keyword, setKeyword, scrollto, setNotification, setNotifibool, notifibool, morefilters, edit} = useContext(ContextApp)
  ...
  return (
    <div className='body'>
     <div className="top" style={{position: 'absolute', top: '-100px'}} id='top'></div>
      <Navbar setKeyword={setKeyword} keyword={keyword} handleLogout={handleLogout}/>
      <Routes keyword={keyword} setKeyword={keyword}/>
      <Quickmenu length={notifilength}/>
      <CSSTransition in={morefilters} unmountOnExit timeout={300} classNames='morefilters'>
        <Morefilters />
      </CSSTransition>
      {noti}
    </div>
