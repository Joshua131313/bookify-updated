...
import { BrowserRouter as Router,Switch,Route,Link, Redirect, useLocation, NavLink } from "react-router-dom" 
...
function Quickmenu(props){
  ...
  const {length} = props
  return (
    <HideElementLink link='/settings' html={
    ...
    <div className="left">
      ...
      {location.pathname.slice(0, 7)!=='/create'?<div className="middle">
        <NavbarLink link='/create' icon='fal fa-plus' />
    </div>:''}
    <div className="right">
      <NavbarLink link='/notifications' icon='fal fa-bell' length={length>100?"!":length}/>
